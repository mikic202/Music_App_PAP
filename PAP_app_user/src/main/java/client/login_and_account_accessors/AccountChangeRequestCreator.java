...
public class AccountChangeRequestCreator {
    public static JSONObject create_send_account_request(String email, String nickname, char[] old_password,
        ...
        return new JSONObject(
                String.format(
                        ...
                        AccountChangeRequestTypes.SEND_DATA.value(), email, nickname, new String(old_password),
    ...
    public static JSONObject create_send_email_request(String email, int user_id) {
        return new JSONObject(
                String.format("{\"type\":\"%s\", \"value\":{\"email\":%s, \"user_id\":%d}}",
                        LoginRequestTypes.SEND_CHANGE_EMAIL.value(), email, user_id));
    }

    public static JSONObject create_send_nickname_request(String nickname, int user_id) {
        return new JSONObject(
                String.format("{\"type\":\"%s\", \"value\":{\"nickname\":%s, \"user_id\":%d}}",
                        LoginRequestTypes.SEND_CHANGE_NICKNAME.value(), nickname, user_id));
    }

    public static JSONObject create_send_new_password_request(char[] new_password, char[] old_password, int user_id) {
        return new JSONObject(
                String.format(
                        "{\"type\":\"%s\", \"value\":{\"new_password\":\"%s\", \"old_password\":\"%s\", \"user_id\":%d}}",
                        LoginRequestTypes.SEND_CHANGE_PASSWORD.value(), new String(new_password),
                        new String(old_password), user_id));
    }
